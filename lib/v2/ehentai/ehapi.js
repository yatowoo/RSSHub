...
async function parsePage(cache, data, get_bittorrent = false, embed_thumb = false) {
    ...
    async function parseElement(cache, element) {
        ...
        if ('mpl'.includes(layout)) {
            ...
            // Other info.
            el_meta = el.find('div.gl3e');
            el_tags = el.find('div.gl4e');
            el_description = el_meta.html() + el_tags.html();
        } else if (layout === 't') {
            // Thumbnail
        ...
        const description = `<img src='${thumbnail}' alt='thumbnail'>` + el_description;
        if (title && link) {
...
async function getFavoritesItems(cache, page, favcat, inline_set, get_bittorrent = false, embed_thumb = false) {
    // set to extended layout
    await ehgot(`favorites.php?favcat=${favcat}&inline_set=dm_e`);
