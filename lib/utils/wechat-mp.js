...
const detectOriginalArticleUrl = ($) => {
    ...
    if ($('#js_content').text().length < 80) {
        // Edge case - breaking news with brief info.
        // example: https://mp.weixin.qq.com/s/8DmyX-YDRZwONxbXEGCxbw
        // '#js_content'.text().length -> 79
        // '#js_content a[href]' -> https://zsdq.dqdaily.com/app/app.html
        const urlObj = new URL($('#js_content a').attr('href'));
        if (urlObj.host !== 'mp.weixin.qq.com') {
            return null;
        } else {
            return urlObj.href;
        }
